...
TORCH_DIR = os.path.join(DRIVERS_DIR, "pytorch_src/torch")
CORTEX_DIR = os.path.join(TALOS_HOME, "cognitive_plane/cortex")

def write_cpp_bridge():
    """ 
    THE OUROBOROS EVENT 
    Generates the Free-Threaded C++ Cortex Substrate dynamically.
    [PATCHED: Includes C-1 Tensor Alignment, Temporal Sparsity, and Transistor Rest]
    """
    cpp_code = r"""
#include <Python.h>
...
"""
    bridge_path = os.path.join(DRIVERS_DIR, "talos_core.cpp")
    with open(bridge_path, "w") as f: 
        f.write(cpp_code)
    return bridge_path

def compile_bridge(cpp_path):
    os.makedirs(CORTEX_DIR, exist_ok=True)
    py_inc = subprocess.check_output("python3-config --includes", shell=True).decode().strip()
    output_so = os.path.join(CORTEX_DIR, "talos_core.so")
    
    torch_inc = os.path.join(TORCH_DIR, "include")
    torch_api_inc = os.path.join(torch_inc, "torch/csrc/api/include")
    torch_lib = os.path.join(TORCH_DIR, "lib")
    rocm_lib = "/home/croudabush/rocm-native/lib"
    rocm_lib64 = "/home/croudabush/rocm-native/lib64"
    
    cmd = [
        # [PHASE 5 FIX: EXPOSING THE WMMA INTRINSICS]
        # We must force the compiler to target the specific gfx1151 architecture
        # to unlock the RDNA 3.5 hardware-native matrix extensions.
        "hipcc", "-O3", "-shared", "-fPIC", "-std=c++17", "-DPy_GIL_DISABLED=1", "-D__HIP_PLATFORM_AMD__=1", 
        "--offload-arch=gfx1151", cpp_path, "-o", output_so,
        *py_inc.split(), f"-I{torch_inc}", f"-I{torch_api_inc}",
        f"-L{torch_lib}", f"-L{rocm_lib}", f"-L{rocm_lib64}",
        f"-Wl,-rpath={torch_lib}", f"-Wl,-rpath={rocm_lib}", f"-Wl,-rpath={rocm_lib64}",
        "-ltorch", "-ltorch_cpu", "-lc10", 
        "-lamdhip64", "-lhsa-runtime64", 
        "-lrocsparse", "-lhipsparse", "-lrocblas", "-lhipblas", 
        "-lrt"
    ]
    
    print("\033[93m[FORGE] Compiling talos_core.so Bridge (Free-Threaded C-API)...\033[0m")
    try:
        subprocess.run(cmd, check=True)
        print(f"\033[92m[SUCCESS] Bridge forged at {output_so}\033[0m")
    except subprocess.CalledProcessError as e:
        print("\033[91m[FATAL] Compilation failed. Review ROCm linker errors.\033[0m")
        sys.exit(1)

if __name__ == "__main__":
    cpp_file = write_cpp_bridge()
    compile_bridge(cpp_file)
